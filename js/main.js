...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Floating contact: one FAB opens panel with "Quote" + "WhatsApp" (all pages with #floating-contact)
    (function initFloatingContact() {
        var root = document.getElementById('floating-contact');
        var toggle = document.getElementById('floating-contact-toggle');
        if (!root || !toggle) return;
        var openKey = 'common.floatingMenuOpen';
        var closeKey = 'common.floatingMenuClose';
        function tr(k) {
            return window.i18n && window.i18n.t ? window.i18n.t(k) : k;
        }
        function setAria() {
            var on = root.classList.contains('is-open');
            toggle.setAttribute('aria-label', tr(on ? closeKey : openKey));
        }
        ...
        document.addEventListener('languageChanged', setAria);
        root.querySelectorAll('a.floating-contact__pill').forEach(function (a) {
